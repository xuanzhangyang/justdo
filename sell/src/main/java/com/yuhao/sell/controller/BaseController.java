...
public class BaseController {
    ...
    public ModelAndView Modification(){
        return new ModelAndView("admin/modification");
    }

    @GetMapping("/search")
    public ModelAndView Search(@RequestParam("dname")String dname,
    ...
    public ModelAndView onModify(@RequestParam("nname")String name,
                              @RequestParam("phone")String phone,
                              @RequestParam("oldname")String oldname,
                              @RequestParam("npassword")String password,
                              Map<String, Object> map){

        if (StringUtils.isEmpty(phone)||StringUtils.isEmpty(name)||StringUtils.isEmpty((oldname))||StringUtils.isEmpty(password)){
            map.put("msg", "信息为空，请填写再试!");
            map.put("url", "/sell/admin/Modification");
            return new ModelAndView("common/error");
        }
        Admin admin = adminService.findByUsername(oldname);
        if (admin==null){
            map.put("msg", "没有此用户");
            map.put("url", "/sell/admin/Modification");
            return new ModelAndView("common/error");
...
//            redisTemplate.opsForValue().getOperations().delete(phone);
            ...
            admin.setPassword(passwordEncoder.encodePassword(password,name));
//            sellerInfo.setPassword(passwordEncoder.encodePassword(password,sellerInfo.getUsername()));
    ...
    public ModelAndView adminLogin(@RequestParam("username")String username,
        ...
        return new ModelAndView("redirect:" + projectUrlConfig.getSell() + "/sell/admin/alllist");
    ...
    public ModelAndView sellerRegister(@Valid SellerInfo sellerInfo,
        ...
        if (multipartFile!=null&&!"".equals(multipartFile.getOriginalFilename())){
            if (multipartFile.getSize()>1148576){
                ...
                try {
                    sellerInfo.setSellerIcon(ImageUtils.uploadToCDNAndGetUrl(multipartFile.getBytes()));
                }catch (Exception e){
                    map.put("url", "/sell/seller/onSellerRegister");
                    map.put("msg", "图片上传失败");
                    return new ModelAndView("common/error", map);
        ...
        try {
            ...
            sellerInfo.setBusinessLicense(idcard);
            sellerInfo.setPoint( (float) 0.007 );
        ...
        return new ModelAndView("common/register_success", map);
    }
